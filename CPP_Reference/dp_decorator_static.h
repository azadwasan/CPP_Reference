...
//One of the magic of static decorator is that one can access the members of the decorated classes.
...
namespace db_decorator_static {
	...
	template <typename T>
	class ColoredShape : public T {	//Mixin inheritance
	...
	class TransparentShape : public T {	//Mixin inheritance
		//static_assert(is_base_of<Shape, T>::value, "Template argument must be a shape");//Ensure its Shape type.
	...
	void main() {
		...
		ColoredShape<TransparentShape<Circle>> red_trans_circle{ "Red", 111, 12 };
		red_trans_circle.resize(3);
